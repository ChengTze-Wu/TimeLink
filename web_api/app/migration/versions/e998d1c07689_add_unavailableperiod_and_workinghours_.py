...
def downgrade() -> None:
    ...
    op.add_column(
        "service",
        sa.Column("period_time", sa.INTEGER(), autoincrement=False, nullable=True),
    )
    op.add_column(
        "service",
    ...
    op.add_column(
        "service", sa.Column("end_date", sa.DATE(), autoincrement=False, nullable=True)
    )
    op.create_table(
        "unavailable_period",
        ...
        sa.ForeignKeyConstraint(
            ["service_id"], ["service.id"], name="unavailable_period_service_id_fkey"
        ),
