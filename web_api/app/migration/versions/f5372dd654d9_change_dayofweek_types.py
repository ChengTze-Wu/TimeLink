...
def upgrade() -> None:
    ...
    dayofweek = sa.Enum('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday', name='dayofweek')
    dayofweek.create(op.get_bind())

    # add USING clause to provide conversion expression
    op.execute("ALTER TABLE working_hour ALTER COLUMN day_of_week TYPE dayofweek USING day_of_week::text::dayofweek")
