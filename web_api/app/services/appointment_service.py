...
class AppointmentService:
    ...
    def create_one(self, appointment_json_data: dict) -> dict:
        ...
        user_id = self.__convert_line_user_id_to_user_id(line_user_id) if line_user_id else self.payload.get("sub")

        if user_id is None:
            raise BadRequest("user_id must be provided in JWT sub or line_user_id be provided in request body")
    ...
    def update_one(self, appointment_id: str, appointment_json_data: dict) -> dict:
        ...
        line_user_id = appointment_json_data.get("line_user_id")
        user_id = self.payload.get("sub") if self.payload.get("sub") else self.__convert_line_user_id_to_user_id(line_user_id)
        role = self.payload.get("role")

        if user_id is None:
            raise BadRequest("user_id must be provided in JWT sub or line_user_id be provided in request body")

        appointment = self.appointment_repository.select_one_by_fields(appointment_id=appointment_id)
    ...
    def cancel_one(self, appointment_id: str, line_user_id: str | None = None) -> dict:
        ...
        if user_id is None:
            raise BadRequest("user_id must be provided in JWT sub or be provided line_user_id in query string")
    ...
    def get_one(self, appointment_id: str) -> dict:
        ...
        return self.appointment_repository.select_one_by_fields(appointment_id=appointment_id)
    ...
    def get_all(
        ...
        if role == RoleName.GROUP_OWNER.value:
            ...
            appointments = [appointment for service_id in service_ids for appointment in self.appointment_repository.select_all_by_filter(service_id=service_id)]
        elif role == RoleName.ADMIN.value:
            appointments = self.appointment_repository.select_all_by_filter(service_id=service_id)
        else:
            appointments = self.appointment_repository.select_all_by_filter(user_id=user_id, service_id=service_id)
    ...
    def __check_service_availability(self, service_id: str, reserved_at: str):
        ...
        if working_hours is None or working_period is None:
            raise BadRequest("The service does not have working hours or working period, please contact the owner")
    ...
    def __check_service_in_user_group(self, service_id: str, user_id: str):
        ...
        service = self.service_repository.select_one_by_unique_filed(service_id=service_id)
        ...
        service_group_ids = set(service_group.get("id") for service_group in service_groups)
    ...
    def __check_appointment_belongs_owner_groups(self, appointment_id: str, owner_id: str):
